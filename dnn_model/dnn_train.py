...
from dnn_model.exporter import BestExporter, auc_bigger
...
def create_feature_columns(note_emb_size=10, note_user_emb_size=6):
    ...
    creator_ids = fc.categorical_column_with_hash_bucket("last_note_creators", hash_bucket_size=2000, dtype=tf.string)#mark1
    note_ids = fc.categorical_column_with_hash_bucket("last_note_ids", 20000, dtype=tf.int64)#mark2

    creator_id = fc.categorical_column_with_hash_bucket("note_open_id", 2000)#mark3
    note_id = fc.categorical_column_with_hash_bucket("note_id", 20000, dtype=tf.int64)#mark4

    video_duration = fc.numeric_column("note_video_duration")#mark5
    video_duration_bucket = fc.bucketized_column(source_column=video_duration,boundaries=[5, 10, 30, 60])
